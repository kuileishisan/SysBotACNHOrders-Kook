...
namespace SysBot.ACNHOrders
{
    public static class NetUtil
    {
        ...
        {
            var result = new Download<Item[]> { SanitizedFileName = att.Filename };
            if ((att.Size > MultiItem.MaxOrder * Item.SIZE) || att.Size < Item.SIZE)
            {
                result.ErrorMessage = $"{result.SanitizedFileName}: 无效大小。";
                return result;
            }
            ...
            if (items == null)
            {
                result.ErrorMessage = $"{result.SanitizedFileName}: 无效的 nhi 附件。";
                return result;
            }
